...
interface IUniswapV3Batcher {
    ...
    /// @return bool Result of collect fees
    function collect(uint256[] calldata tokenIds) external payable returns (bool);

    struct CollectAndCloseParams {
        uint256 tokenId;
        uint256 amount0Min;
        uint256 amount1Min;
        uint256 deadline;
    ...
    /// be approved by `msg.sender` preferably using `setApprovalForAll`
    /// @param params The ids of the tokens with slippage tolerances encoded as `CollectAndCloseParams` calldata
    /// @return bool Result of collect and close
    function collectAndClose(CollectAndCloseParams[] calldata params) external payable returns (bool);
    ...
    /// @param balances The balances necessary of each token to mint the positions encoded as `BalanceParams` in calldata
    /// @param collectParams The tokens to close the position encoded as `CollectAndCloseParams` in calldata
    /// @return tokenIds The NF token Ids of the newly minted positions
    function rerange(MintParams[] calldata mintParams, BalanceParams[] calldata balances, CollectAndCloseParams[] calldata collectParams) external payable returns (uint256[] memory tokenIds);
